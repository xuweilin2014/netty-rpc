public class DubboAdaptive {
     ...
     * 当@Adaptive注解在接口方法上时，表示拓展的加载逻辑需由框架自动生成
     * 当@Adaptive注解在类上时，表示拓展的加载逻辑需要由人工编码完成
     */
    @Documented
    ...
    public class ExtensionLoader{
        ...
        private T createAdaptiveExtension() {
            try {
                ...
                //Dubbo 中有两种类型的自适应拓展，一种是手工编码的，一种是自动生成的。手工编码的自适应拓展中可能存在着一些依赖，
        ...
        private Class<?> getAdaptiveExtensionClass() {
            // 通过 SPI 获取所有的拓展类，也就是获取到某个接口的全部实现类，比如该方法可以获取 Protocol 接口的
            // DubboProtocol、HttpProtocol、InjvmProtocol 等实现类。在获取实现类的过程中，如果某个实现类被 Adaptive 注解修饰了，
            // 那么该类就会被赋值给 cachedAdaptiveClass 变量，下面一步中就可以直接返回获取到的cachedAdaptiveClass。如果
            // 如果所有的实现类均未被 Adaptive 注解修饰，那么执行下面第三步逻辑，创建自适应拓展类。
            getExtensionClasses();
            // 检查缓存，若缓存不为空，则直接返回缓存
        ...
        private String createAdaptiveExtensionClassCode() {
            ...
            codeBuilder.append("\npublic class ").append(type.getSimpleName()).append("$Adaptive").append(" implements ")
            ...
            for (Method method : methods) {
                ...
                if (adaptiveAnnotation == null) {
                    ...
                    // 遍历 value，这里的 value 是 Adaptive 的注解值，2.2.3.3 节分析过 value 变量的获取过程。此处循环目的是生成从 URL 中获取拓展名的代码，
