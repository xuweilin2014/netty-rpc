    ...
    public static HessianSerializePool getHessianPoolInstance() {
        if (poolFactory == null) {
            synchronized (HessianSerializePool.class) {
                if (poolFactory == null) {
                    poolFactory = new HessianSerializePool(RpcConfig.SERIALIZE_POOL_MAX_TOTAL, RpcConfig.SERIALIZE_POOL_MIN_IDLE, RpcConfig.SERIALIZE_POOL_MAX_WAIT_MILLIS, RpcConfig.SERIALIZE_POOL_MIN_EVICTABLE_IDLE_TIME_MILLIS);
                }
            }
        }
        return poolFactory;
