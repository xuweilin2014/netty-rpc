...
public class MessageCallBack {
        ...
        try {
            ...
            if (this.response != null) {
                if (response.getInvokeStatus().isRejected()){
                    throw new RejectResponeException(RpcConfig.FILTER_RESPONSE_MSG);
    ...
    private void await() {
        ...
        try {
            ...
            isTimeout = finish.await(RpcConfig.SYSTEM_PROPERTY_MESSAGE_CALLBACK_TIMEOUT, TimeUnit.MILLISECONDS);
        ...
        if (!isTimeout) {
            throw new InvokeTimeoutException(RpcConfig.TIMEOUT_RESPONSE_MSG);
        }
    }
