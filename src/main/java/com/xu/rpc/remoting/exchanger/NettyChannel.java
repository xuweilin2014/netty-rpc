...
import com.xu.rpc.commons.RpcParameterLock;
...
public class NettyChannel implements RpcChannel {
    ...
    public static RpcChannel getChannel(Channel channel, URL url){
        ...
        if (!CHANNELS.containsKey(channel)){
            // 对于每一个不同的 channel，会得到不同的 lock 对象，而对于相同的 channel，则【可能】会得到相同的的 lock，这是因为弱引用的原因
            Lock lock = RpcParameterLock.getParameterLock(channel);
            lock.lock();
            try{
                if (!CHANNELS.containsKey(channel)) {
