...
public class AsyncCallResult {
    ...
    /**
     * 我们在AsyncRpcTimeoutCallTest中把要执行的异步任务最终封装成一个FutureTask，丢到线程池中去执行（具体的就是调用FuturetTask中的
     * run方法）。以AsyncRpcTimeoutCallTest为例，线程池中的线程会首先执行calculate.busy方法，然后进入到MessageSendProxy去向RPC服务器
     * 发送请求方法，接着调用callBack.start()阻塞，等待调用的结果返回。默认等待的时间为30s，如果等待超时，就会抛出自定义的InvokeTimeoutException
     * 异常。这个异常在FutureTask的run方法中会被捕获，并且通过setException设置给outcome属性。这是线程池中的线程所做的事情。
     *
     * 在AsyncRpcTimeoutCallTest中的main线程中，调用代理对象的方法（除了_getStatus之外），会调用下面的loadFuture方法来获取上面线程池执行
     * 任务返回的对象。future.get(long timeout, TimeUnit unit)方法返回juc包中的TimeoutException，必须是在state一直为NEW和COMPELTEING
